...
class Matcher {
  ...
  makePayload(matches) {
    ...
    if (routeMatch.groups) {
      process.env.NODE_ENV !== "production" ? (0, _tinyWarning.default)(matches.length === 1, 'Route match with groups %s has children, which are ignored.', Object.keys(routeMatch.groups).join(', ')) : void 0;
      const groupRouteIndices = {};
      const routeParams = [];
      const _params = {};
      Object.entries(routeMatch.groups).forEach(([groupName, groupMatches]) => {
        const groupPayload = this.makePayload(groupMatches); // Retain the nested group structure for route indices so we can
        ...
        routeParams.push(...groupPayload.routeParams); // Just merge all the params depth-first; it's the easiest option.
  ...
  isPathnameActive(matchPathname, pathname, exact) {
    ...
    const pathnameWithSeparator = pathname.slice(-1) !== '/' ? `${pathname}/` : pathname; // Can't use startsWith, as that requires a polyfill.
