...
function renderError(error, options) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    try {
      var _renderStackTrace = renderStackTrace(error, options),
        _renderStackTrace$sta = _renderStackTrace.status,
        status = _renderStackTrace$sta === void 0 ? 500 : _renderStackTrace$sta,
        content = _renderStackTrace.content;
      if (content) {
        return {
          status: status,
          content: content,
...
function renderStackTrace(error, options) {
  ...
  if (typeof error.status === 'number') {
    ...
    if (error.response && error.response.headers['content-type'] && error.response.headers['content-type'].split(';')[0].trim() === 'text/html') {
      return {
        status: error.status,
